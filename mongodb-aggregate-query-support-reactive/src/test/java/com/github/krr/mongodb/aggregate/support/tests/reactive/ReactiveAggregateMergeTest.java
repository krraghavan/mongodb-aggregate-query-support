...
import com.github.krr.mongodb.aggregate.support.repository.reactive.ReactiveZooEmployeeRepository;
...
public class ReactiveAggregateMergeTest extends AbstractTestNGSpringContextTests {
  ...
  @Autowired
  private ReactiveZooEmployeeRepository zooEmployeeRepository;
  ...
  private List<Employee> addEmployeeDocuments(String collectionName, String coll) throws JsonProcessingException {
    ...
    mongoTemplate.insert(employees, collectionName).blockLast();
    return employees;
  }
  ...
  private List<Salary> addSalaries(String collectionName, String coll) throws JsonProcessingException {
    ...
    mongoTemplate.insert(salaries, collectionName).blockLast();
    return salaries;
  }
  ...
  public void mergeMustMergeObjectsIntoOutRepository() throws JsonProcessingException {
    ...
    mongoTemplate.insert(addEmployeeDocuments("employee", EMPLOYEE_DOCS), employeeCollection).blockLast();
    zooEmployeeRepository.createBudgetsCollection(employeeCollection, budgetCollection).block();
    mongoTemplate.count(new Query(), Budget.class, budgetCollection).block();
  ...
  private void validateBudget(List<Budget> budgets, Flux<Budget> expectedBudgetFlux) {
    List<Budget> expectedBudgetDocs = expectedBudgetFlux.collectList().block();
    assertNotNull(expectedBudgetDocs);
  ...
  public void mergeMustCreateOnDemandMaterializedViews() throws JsonProcessingException {
    ...
    zooEmployeeRepository.createBudgetsIntoCollection(employeeCollection, budgetsCollection).block();
    List<Budget> expectedBudgetDocs = getBudgets(EXPECTED_BUDGETS_DOCS);
    Query query = new Query().with(Sort.by("_id"));
    ...
    validateCount(employeeCollection, originalEmployees.size() + newEmployees.size());
    zooEmployeeRepository.replaceBudgets(employeeCollection, budgetsCollection).block();
  ...
  private void validateCount(String employeeCollection, int numEmployees) {
    Long employeeCount = mongoTemplate.count(new Query(), Employee.class, employeeCollection).block();
    assertNotNull(employeeCount);
    assertEquals((long) employeeCount, numEmployees);
  }
