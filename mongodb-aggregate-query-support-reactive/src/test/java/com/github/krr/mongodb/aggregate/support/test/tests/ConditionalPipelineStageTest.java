...
import static com.github.krr.mongodb.aggregate.support.test.utils.FixtureUtils.createPossessions;
import static com.github.krr.mongodb.aggregate.support.test.utils.FixtureUtils.createPossessionsWithSortField;
...
public class ConditionalPipelineStageTest extends AbstractTestNGSpringContextTests {
  ...
  public void mustConditionallyExcludePipelineStagesMutuallyExclusiveCase() {
    ...
    possessionsRepository.saveAll(Arrays.asList(expectedCarPossessions, expectedHomePossessions)).collectList().block();
  ...
  public void mustReturnResultWithMixOfConditionalAndUnconditionalStages() {
    ...
    List<Possessions> carsOnlyPossessions = possessionsRepository.getWithMixOfConditionalAndUnconditionalStages(tag, true,
  ...
  public void mustReturnResultWithPotentiallyNullIdList() {
    ...
    List<Possessions> allPossessions = possessionsRepository.getPossessionsWithPotentiallyNullIdList(tag,null)
                                                            .collectList().block();
    LOGGER.info("allPossessions:{}", allPossessions);

    verifyPossessions(expectedAllPossessions, allPossessions);

    List<Possessions> carPossessions = possessionsRepository.getPossessionsWithPotentiallyNullIdList(tag, carIds)
                                                            .collectList().block();
  ...
  public void mustApplyOrAndOnConditionalsWhenConditionType() {
    ...
    for (int i=0; i<10; i++) {
      possessionsRepository.save(createPossessions(true, false, tag)).block();
    }
    ...
    assertEquals(possessions.size(), 4);
    ...
    possessions = possessionsRepository.getPosessionsBasedOnConditionType(tag, false, false)
                                       .collectList().block();
    assertNotNull(possessions);
    assertEquals(possessions.size(), 8);
  }
}
