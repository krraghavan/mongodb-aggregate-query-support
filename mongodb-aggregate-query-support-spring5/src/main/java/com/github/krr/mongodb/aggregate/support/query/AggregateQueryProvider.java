 ...
 * 4/1/17.
 */
@SuppressWarnings({"squid:S1067", "Duplicates", "rawtypes"})
public class AggregateQueryProvider extends AbstractAggregateQueryProvider<Pageable> {
  ...
  private static final int B_GENERAL = 0;
  ...
  private String getParameterValueForBinding(ConvertingParameterAccessor accessor, ParameterBinding binding) {
    ...
    if(Collection.class.isAssignableFrom(value.getClass())) {
      // process each entry in the collection.  Finally the value in the collection is
      // returned as a JSON array "[v1, v2, ...]"
      ...
      for (Object o : (Collection) value) {
        ...
        builder.append(getValue(o, binding));
      }
      builder.append("]");
      return builder.toString();
    }
    else {
      return getValue(value, binding);
    }
  }

  private String getValue(Object value, ParameterBinding binding) {
    boolean stripQuotes = binding.stripQuotes();
    if (value instanceof String && binding.isQuoted() && !stripQuotes) {
      return (String)value;
    }
    if (value instanceof String && binding.isQuoted() && stripQuotes) {
    ...
    if (value instanceof String && !binding.isQuoted() && !stripQuotes) {
      // string value is not quoted - add quotes
      return getQuotedString(value);
    }
  ...
  private String getQuotedString(Object value) {
    return "\"" + value.toString() + "\"";
  }
  ...
  static class NonReactiveParameterBindingParser extends ParameterBindingParser  {
